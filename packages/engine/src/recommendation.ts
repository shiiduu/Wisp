...
function scoreItemForTag(item: Item, tag: BuildTag): number {
  ...
  return overlap * 10 * (TIER_WEIGHT[tier] ?? 0);
}

/**
 * Ranks completed (non-boots, non-consumable) items by fit for a build tag.
 * Data Dragon lists separate ids for the same item across game modes (e.g.
 * a Summoner's Rift and an Arena-remixed "Archangel's Staff" share a name
 * but differ in id/price) — dedupe by name, keeping the highest-scoring
 * variant, so the same item never appears twice in one recommendation.
 */
export function recommendItemsForTag(items: Item[], tag: BuildTag): Item[] {
  const scored = items
    .map((item) => ({ item, score: scoreItemForTag(item, tag) }))
